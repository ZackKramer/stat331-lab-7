...
fish |>
  summarize(
    na_obs = sum(is.na(across(everything())) > 0),
    na_vars = names(select(cur_data(), where(~ any(is.na(.)))))
  )
...
fish |>
  ...
  summarize(na_freq = sum(is.na(across(everything())))) |>
...
in the weight calculation, min(length, na.rm = TRUE) is incorrectly used instead of min(weight, na.rm = TRUE)
